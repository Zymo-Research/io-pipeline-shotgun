process FASTP {
    ...
    if ( task.ext.args?.contains('--interleaved_in') ) {
        """
        [ ! -f  ${prefix}_R1.fastq ] && ln -sf $reads ${prefix}_R1.fastq

        fastp \\
            --stdout \\
            --in1 ${prefix}_R1.fastq \\
            --thread $task.cpus \\
        ...
        """
        [ ! -f  ${prefix}_R1.fastq ] && ln -sf ${reads[0]} ${prefix}_R1.fastq
        [ ! -f  ${prefix}_R2.fastq ] && ln -sf ${reads[1]} ${prefix}_R2.fastq
        fastp \\
            --in1 ${prefix}_R1.fastq \\
            --in2 ${prefix}_R2.fastq \\
            --out1 ${prefix}_R1.fastp.fastq \\
            --out2 ${prefix}_R2.fastp.fastq \\
            --json ${prefix}.fastp.json \\
            --html ${prefix}.fastp.html \\
            $adapter_list \\
            $fail_fastq \\
