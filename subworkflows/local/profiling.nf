    ...
    */
    ...
    if ( params.profiler == "metaphlan4" ) {
        ...
        ch_input_for_metaphlan4 = ch_input_for_profiling
                            .filter{
                                if (it[0].is_fasta) log.warn "[Zymo-Research/io-pipeline-shotgun] MetaPhlAn4 currently does not accept FASTA files as input. Skipping MetaPhlAn4 for sample ${it[0].id}."
                                !it[0].is_fasta
                            }
                            .multiMap {
                                it ->
    ...
    if ( params.profiler == "sourmash" ) {
        ...
        ch_input_for_sourmash =  ch_input_for_profiling
                                .filter{
                                    if (it[0].is_fasta) log.warn "[Zymo-Research/io-pipeline-shotgun] Sourmash currently does not accept FASTA files as input. Skipping Sourmash for sample ${it[0].id}."
                                    !it[0].is_fasta
                                }
                                .multiMap {
                                    it ->
